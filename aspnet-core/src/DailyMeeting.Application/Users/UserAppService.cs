...
namespace DailyMeeting.Users
{
    ...
    public class UserAppService : AsyncCrudAppService<User, UserDto, long, PagedUserResultRequestDto, CreateUserDto, UserDto>, IUserAppService
    {
        ...
        [HttpPost]
        public async Task<string> UploadAvatar([FromForm] FileInputDto input)
        {
            var path = Path.Combine(_hostingEnvironment.WebRootPath, "avatars");
            ...
            if (input != null && input.File != null && input.File.Length > 0)
            {
                ...
                if (FileExtension == "jpeg" || FileExtension == "png" || FileExtension == "jpg" || FileExtension == "gif")
                {
                    ...
                    var fileImage = Guid.NewGuid().ToString() + ".png";
                    using (var stream = System.IO.File.Create(Path.Combine(_hostingEnvironment.WebRootPath, "avatars", fileImage)))
                    {
                        await input.File.CopyToAsync(stream);
                        user.AvatarPath = "/avatars/" + fileImage;
                        await _userManager.UpdateAsync(user);
                    }
                    return "/avatars/" + fileImage;
                }
                else
                {
